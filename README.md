...
Em um projeto que já executou o esquema anteriormente, execute também [`supabase/brands-migration.sql`](supabase/brands-migration.sql), [`supabase/product-order-migration.sql`](supabase/product-order-migration.sql) e [`supabase/colors-migration.sql`](supabase/colors-migration.sql) uma vez. As migrações criam marcas, ordenação manual, cores, imagens por variação e o registro da cor escolhida nos pedidos.
...
As tabelas `products`, `categories`, `brands`, `colors` e `product_colors` entram na publicação `supabase_realtime` para atualizar a vitrine em outras abas. A loja também refaz a consulta ao voltar para a aba. O esquema promove o usuário principal já criado para `profiles.role = 'admin'`. Para promover outro usuário, execute `update public.profiles set role = 'admin' where id = 'UUID';`. Não inclua a service role no frontend.
